...
$(function(){
	...
	var content_h = $("#manager-banner").height()+$("#quality").height() + $("header").height();
	...
	function gotoReg() {
		var input_top = content_h - $("#apply").height() - $("#features-1").height()/2 - parseInt($("#features-1").css("margin-bottom"));
		if(device == "apple")
		{
    ...
    function checkForms() {
        ...
        $.ajax({
        	type: "GET",
        	url: "http://www.uzhuang.com/api/wap.php?action=fbform&title="+$("#user-name").val()+"&telephone="+$("#user-pwd").val()+"&source="+$("#source").val()+"&time="+new Date().getTime(),
        	dataType: "json",
        	beforeSend: function(){
    ...
    function getPerson(){
    	$.ajax({
    		type: "GET",
    		url: "http://www.uzhuang.com/api/wap.php?action=fbcount",
    		dataType: "text",
