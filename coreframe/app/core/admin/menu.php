...
class menu extends WUZHI_admin {
	 ...
	 */
	public function add() {
		   $result = $this->db->get_list('site', $where, '*', 0, 100, 0, '');
        if(isset($GLOBALS['submit']) || isset($GLOBALS['submit2'])) {
            $GLOBALS['form']['site']= ",".implode(',',$GLOBALS['site']).",";
            $formdata = array();
            ...
            $formdata['site'] = $GLOBALS['form']['site'];
    ...
    public function edit() {
        ...
        $siteid = $this->db->get_one('menu',array('menuid'=>$id) ,'site');
        $sites = explode(',',$siteid['site']);
        if(!$id) MSG(L('参数错误'));
