...
class order extends WUZHI_foreground {
//订单列表
   public function listing(){
       $uid = get_cookie('uid');
       // $uid=191;
       $where="`uid`=$uid AND `status`=9";
       $result=$this->db->get_list('demand',$where,'title,id,order_no,address',0,1000,'id desc');
       $arr=array();
       $arr1=array();
       foreach ($result as $key => $value) {
          ...
          $arr[$value['id']]=array('id'=>$value['id'],'kaigongriqi'=>null,'bianhao'=>$value['order_no'],'zuixinjindu'=>$res[0]['nodename'],'dizhi'=>$value['address']);
           // $arr1=array($res[0]);
           //查工程开工天数
           $res1=$this->db->get_list('demand_track',"`orderid`=$s",'orderid,userid,nodeid,nodename,addtime',0,1000);
           foreach ($res1 as $key => $value1) {
               if ($value1['nodeid']==21) {
                               $nowTime=date('Y-m-d H:i:s',time());
                               $addTime=$value1['addtime'];
                               $time=(strtotime($nowTime)-strtotime($addTime))/86400;
                               $time=floor($time);
                               $arr[$value1['orderid']]['kaigongriqi']=$time;
                                   }
                             }
                       }
