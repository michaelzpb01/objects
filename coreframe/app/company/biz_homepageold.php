...
class biz_homepage extends WUZHI_foreground {
	...
	public function listing() {
        ...
        $result2 = $this->db->get_list('picture',"`companyid`=$uid".$where, '*', 0, 3,$page,'id');
       
        $result3 = $this->db->get_list('company_team',"`companyid`=$uid".$where, '*', 0, 3,$page,'id');
      
        $result4= $this->db->get_list('demand_fina',"`iscompany`='$com' AND `companyid`=$uid", '*', 0, 10,$page,'paydate desc');
          
               foreach ($result4 as $key => $value) {
                if(isset($aCom2row[$value['order_no']])){
                  $aCom2row[$value['order_no']]['totalpay'] += $value['totalpay'];
                            }else{
